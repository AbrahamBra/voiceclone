...
| 2026-04-25 | Adopter le pattern ace-agent/ace `helpful_count` / `harmful_count` par artifact pour piloter le retrait de règles. Migration 049 + lib/protocol-v2-rule-counters.js stub. Wiring callers en follow-up | ace est le repo OSS le plus structurellement proche de VoiceClone (papier arXiv:2510.04618). Sans signal helpful/harmful, le protocole accumule des règles sans jamais en retirer. Voir `audit/architecture-analogues.md` Reco A | Après wiring callers + 50+ firings résolus → mesurer si le seuil 6/0.6 est bon |
| 2026-04-25 | Trait 3 du protocole vivant — changelog narratif lisible non-tech (Sudowrite-style) à publier à chaque v(N+1). Stocker sur `protocol_versions.summary_narrative` | Différenciateur de pitch agence : un setter doit pouvoir lire "ce qui a changé cette semaine et pourquoi" en 30 secondes. Calibrer le ton sur https://feedback.sudowrite.com/changelog | Après 3 publishes réels → A/B la version narrative vs git-log technique |
| 2026-04-25 | Détection de contradictions au routing du drain (semantic + LLM-judge binaire), reportée à après merge PR #123 | Évite que deux règles contradictoires coexistent. Touche `lib/protocol-v2-extractor-router.js` qui est en cours de modification dans PR #123 — collision certaine si fait maintenant | Quand PR #123 est mergée |
