// Source-specific playbooks v2 — list + create.
//
// V2 of the playbook flow shipped in #158 : exposes endpoints to list a
// persona's source-specific protocol_documents and to create a new one from
// a pasted prose (e.g. a Notion doc copied as text). On creation, runs the
// extractor pipeline + auto-materializes high-confidence artifacts so the
// playbook is immediately usable in chat.
//
// Endpoints :
...
//
// Concurrency : one active doc per (persona, source_core). If a fresh POST
// hits an existing pair, returns 409 Conflict — the caller must archive the
// existing doc first (V2.1 will add an archive endpoint, manually for now).
//
...
// ── GET — list source playbooks for a persona ─────────────────────
