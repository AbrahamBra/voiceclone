...
```
Signal → Extracteur LLM (prompt = source + doc current state + 20 props récentes pour dédup)
  → Candidat { intent, target_kind, proposed_text, rationale, confidence }
  → Dédup sémantique (embedding similarity >= 0.85 → MERGE count++)
...
- Template évolue → notification par persona héritée → vue merge 3-way (v_prev hérité / v_new template / v_local persona) → accept/reject par section
...
| 6 | Extracteur self-improvement (training_example, cron régénération) | Après 20 rejets d'un pattern, l'extracteur arrête de le proposer |
