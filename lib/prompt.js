...
const PROTOCOL_TOKEN_BUDGET = 400;
const SEVERITY_WEIGHT = { hard: 3, strong: 2, light: 1 };

/**
 * Render protocol_v2 artifacts as a system-prompt block. Returns the rendered
 * block + the list of artifact IDs that actually fit under the budget — the
 * caller logs those IDs as firings (Chantier 2bis).
 *
 * Distinct from formatProtocolSection (legacy operating_protocols) so the
 * LLM sees them as a separate "recently learned" cohort and so we can phase
 * out the legacy block later without touching this path.
 *
 * @param {Array<{id:string, content:object, severity:string}>} artifacts
 * @returns {{block: string, injectedIds: string[]}}
 */
function formatProtocolArtifactsBlock(artifacts) {
  if (!Array.isArray(artifacts) || artifacts.length === 0) {
    return { block: "", injectedIds: [] };
  }
  let block = "REGLES APPRISES (PROTOCOLE-V2) :\n";
  ...
  for (const a of artifacts) {
    ...
    const line = `${sevTag} ${text.trim()}\n`;
    const tokens = estimateTokens(line);
    if (used + tokens > PROTOCOL_TOKEN_BUDGET) break;
    block += line;
    used += tokens;
    injectedIds.push(a.id);
  }
  if (injectedIds.length === 0) return { block: "", injectedIds: [] };
  return { block: block + "\n", injectedIds };
}

function formatProtocolSection(rules, scenarioSlug) {
  if (!Array.isArray(rules) || rules.length === 0) return "";
  const scoped = rules.filter(r =>
    !r.applies_to_scenarios || r.applies_to_scenarios.length === 0 ||
    (scenarioSlug && r.applies_to_scenarios.includes(scenarioSlug))
  );
  ...
  let block = "PROTOCOLE ACTIF — REGLES ABSOLUES NON NEGOCIABLES :\n";
  let used = estimateTokens(block);
  let n = 0;
  for (const rule of scoped) {
    const scope = rule.applies_to_scenarios?.length
      ? ` [${rule.applies_to_scenarios.join(",")}]`
      : "";
    const line = `${++n}. ${rule.description}${scope}\n`;
    const tokens = estimateTokens(line);
    if (used + tokens > PROTOCOL_TOKEN_BUDGET) { n--; break; }
    block += line;
    used += tokens;
  }
  if (n === 0) return "";
...
export function buildSystemPrompt({ persona, knowledgeMatches, scenarioContent, corrections, ontology, scenarioKind = "dm", protocolRules = [], protocolArtifacts = [], scenarioSlug = null }) {
  ...
  const isPost = scenarioKind === "post";

  // --- ALWAYS INCLUDED: Identity + Voice Rules (~300-500 tokens) ---
  let core = `Tu es ${persona.name}, ${persona.title}.\n`;
  core += `${persona.description}\n\n`;
  // Protocol rules placed BEFORE voice rules: they're deterministic, non-negotiable,
  // and must not be truncated by downstream budget pressure.
  const protocolBlock = formatProtocolSection(protocolRules, scenarioSlug);
  if (protocolBlock) core += protocolBlock;
  // Chantier 2bis — protocol_v2 artifacts (learned rules). Rendered AFTER
  // legacy rules so the older formal block stays first; injectedArtifactIds
  // is returned so api/chat.js can log firings against the persisted message.
  const { block: artifactsBlock, injectedIds: injectedArtifactIds } =
    formatProtocolArtifactsBlock(protocolArtifacts);
  if (artifactsBlock) core += artifactsBlock;
  core += "REGLES DE VOIX :\n";
  ...
  if (isPost) {
    core += "FORMAT DE REPONSE OBLIGATOIRE (POST LINKEDIN) :\n";
    core += "- Produis UN post LinkedIn complet, en un SEUL bloc. PAS de multi-messages style WhatsApp.\n";
    core += "- Respecte la longueur indiquee par le mode du scenario (en general 800-2500 caracteres selon le sous-type).\n";
    core += "- Structure aeree : paragraphes courts (1-3 lignes), sauts de ligne frequents pour la lecture mobile.\n";
    core += "- Pas de preambule (\"Voici ton post :\") ni d'epilogue explicatif. Juste le post, pret a copier.\n";
    core += "- Pas de header meta (\"**Format :**\", \"**Angle :**\", \"**Cible :**\") dans ta sortie — l'operateur attend le post brut.\n";
    core += "- N'ECRIS JAMAIS les caracteres litteraux \\n, /n, ou un backslash suivi de n. Utilise UN VRAI retour a la ligne.\n\n";
  } else {
  ...
  if (usedTokens > 4000) {
    const formatHint = isPost
      ? "Format post LinkedIn complet, pas WhatsApp"
      : "Style WhatsApp, messages courts";
    parts.push(`RAPPEL : Tu es ${persona.name}. Ton : ${v.tone[0]}. ${formatHint}. Mots interdits : ${v.forbiddenWords.slice(0, 3).join(", ")}.\n`);
  }

  return { prompt: parts.join(""), detectedPages, injectedEntities, injectedCorrectionsCount, injectedArtifactIds };
}
