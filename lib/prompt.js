...
export function buildSystemPrompt(personaId, scenario, messages) {
  ...
  const knowledgeMatches = findRelevantKnowledge(personaId, messages);
  ...
  if (scenarioConfig?.file) {
    const scenarioContent = loadPersonaFile(personaId, scenarioConfig.file);
    if (scenarioContent) {
