...
// Migration 055 — bumped 400 → 1500 to fit a full source-specific playbook
// (Nicolas's visite_profil pulls ~30 artifacts × ~50 tokens = ~1500 tokens).
// At 400, the prompt would only carry 6-8 artifacts and the operational
// content of the playbook (strategy, cadence, mirror questions, do-not list)
// would silently get dropped while the voice overrides ate the whole budget.
// Trade-off: 1100 tokens shifted from knowledge/ontology to protocol — fine
// since protocol rules are foundational and bounded by limit=30.
const PROTOCOL_TOKEN_BUDGET = 1500;
...
export function buildSystemPrompt({ persona, knowledgeMatches, scenarioContent, corrections, ontology, protocolRules = [], protocolArtifacts = [], scenarioSlug = null }) {
