...
// Threshold for considering two synthesized rules near-duplicates (normalized
// Levenshtein similarity). Was a 20-char-prefix substring check, which: (a)
// collapsed any two rules sharing a 20-char prefix even if they diverged
// after, and (b) for rules shorter than 20 chars, slice(0,20) returned the
// whole string, making `includes` match almost anything.
const RULE_DEDUP_SIMILARITY = 0.85;
...
Synthetise en UNE regle. Max 30 mots. Regle seule, pas d'explication.`,
    ...
    // Fallback: most recent correction in the cluster. Callers load with
    // ascending=false (newest first), so cluster.members preserves that order
    // and index 0 is the newest. Off-by-one fix — was returning the OLDEST.
...
export async function consolidateCorrections(personaId, { client = null } = {}) {
  ...
  // Load active corrections. Validation markers ([VALIDATED] /
  // [CLIENT_VALIDATED] / [EXCELLENT]) are entity-boost signals, not rules —
  // they must not enter clustering, otherwise they get synthesized as rules
  // and leak into the Intelligence feed.
  const VALIDATION_MARKER_PREFIXES = ["[VALIDATED]", "[CLIENT_VALIDATED]", "[EXCELLENT]"];
  ...
  const corrections = (rawCorrections || []).filter(c =>
    !VALIDATION_MARKER_PREFIXES.some(p => (c.correction || "").startsWith(p))
  );
  ...
  for (const cluster of promotable) {
    ...
    // Check if a similar rule already exists (normalized Levenshtein ≥ 0.85)
    const alreadyExists = currentRules.some(r => ruleSimilarity(r, synthesized) >= RULE_DEDUP_SIMILARITY);
