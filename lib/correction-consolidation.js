...
import { cosineSim, greedyCluster, calculateFidelityScore } from "./fidelity.js";
import { logLearningEvent } from "./learning-events.js";
import { log } from "./log.js";

// Min members for a cluster to even be considered (was 3).
// Lowered to 2 because MIN_CLUSTER_WEIGHT_SUM now acts as secondary gate —
// two strong explicit corrections (2 × 1.0 × 0.8 = 1.6) still won't promote
// unless they're joined by at least one more signal.
const MIN_CLUSTER_MEMBERS = 2;
// Minimum aggregate weight for a cluster to be promoted.
// Effective weight = confidence × confidence_weight. Examples:
//   3 explicit fresh (3 × 1.0 × 0.8 = 2.40) → promoted (backward compat)
//   2 explicit + 1 copy_paste (2 × 0.80 + 0.48 = 2.08) → promoted
//   5 copy_paste (5 × 0.48 = 2.40) → promoted (requires 5× evidence)
//   2 copy_paste (2 × 0.48 = 0.96) → NOT promoted
const MIN_CLUSTER_WEIGHT_SUM = 2.0;
const DEFAULT_THRESHOLD = 0.75;
const MAX_WRITING_RULES = 25;
const BACKTEST_REVERT_SCORE_DELTA = -5;   // auto-revert if composite score drops by more than 5
const BACKTEST_REVERT_COLLAPSE_DELTA = -8; // auto-revert if collapse index drops by more than 8
const BACKTEST_COLLAPSE_FLOOR = 70;        // only revert on collapse if score_global < this
 ...
 */
export function computeAdaptiveThreshold(embeddings) {
...
/**
 * Greedy clustering of corrections with adaptive per-persona threshold.
 * The `corrections` argument is accepted for backwards compat but only its
 * length matters — clustering operates on embeddings. Returns clusters and
 * the threshold actually used, so callers can log it.
 */
export function clusterCorrections(corrections, embeddings) {
  const threshold = computeAdaptiveThreshold(embeddings);
  const clusters = greedyCluster(embeddings, threshold);
...
export async function consolidateCorrections(personaId, { client = null } = {}) {
  ...
  // Load active corrections (now including confidence_weight from migration 036)
  const { data: corrections } = await supabase
    .from("corrections")
    .select("id, correction, confidence, confidence_weight, source_channel, is_implicit, created_at")
    .eq("persona_id", intellId)
    .eq("status", "active")
    .order("created_at", { ascending: false });

  if (!corrections?.length || corrections.length < MIN_CLUSTER_MEMBERS) {
  ...
  // Promotion gate: both criteria must be met.
  //   1. At least MIN_CLUSTER_MEMBERS (2) corrections to avoid single-signal rules
  //   2. Aggregate effective weight ≥ MIN_CLUSTER_WEIGHT_SUM (2.0)
  // This lets strong explicit signals promote quickly (3× 0.80 = 2.40) while
  // requiring more evidence for implicit ones (5× 0.48 = 2.40).
  const clusterWeight = (cluster) => cluster.members.reduce((sum, i) => {
    const c = corrections[i];
    return sum + (c.confidence ?? 0.8) * (c.confidence_weight ?? 1.0);
  }, 0);

  const promotable = clusters.filter(c =>
    c.members.length >= MIN_CLUSTER_MEMBERS &&
    clusterWeight(c) >= MIN_CLUSTER_WEIGHT_SUM
  );
  ...
  // Cap writingRules — evict LOWEST aggregate-weight rules first to prevent
  // prompt overflow. Aggregate weight = sum(confidence × confidence_weight)
  // of all graduated corrections feeding a rule. Legacy rules (no graduated
  // match) get weight 0 and are evicted first; ties broken by insertion order
  // (oldest first — preserves FIFO as fallback).
  if (currentRules.length > MAX_WRITING_RULES) {
    const overflow = currentRules.length - MAX_WRITING_RULES;

    const { data: gradCorrs } = await supabase
      .from("corrections")
      .select("graduated_rule, confidence, confidence_weight")
      .eq("persona_id", intellId)
      .eq("status", "graduated")
      .in("graduated_rule", currentRules);

    const weights = new Map();
    for (const rule of currentRules) weights.set(rule, 0);
    for (const c of (gradCorrs || [])) {
      const w = (c.confidence ?? 0.8) * (c.confidence_weight ?? 1.0);
      weights.set(c.graduated_rule, (weights.get(c.graduated_rule) ?? 0) + w);
    }

    // Sort by (weight ASC, original index ASC). Pick `overflow` weakest to evict.
    const ranked = currentRules.map((rule, idx) => ({ rule, idx, w: weights.get(rule) ?? 0 }));
    ranked.sort((a, b) => a.w - b.w || a.idx - b.idx);
    const toEvict = new Set(ranked.slice(0, overflow).map(x => x.rule));

    const evictedCount = toEvict.size;
    for (let i = currentRules.length - 1; i >= 0; i--) {
      if (toEvict.has(currentRules[i])) currentRules.splice(i, 1);
    }

    log("writing_rules_evicted", {
      persona: intellId,
      evicted_count: evictedCount,
      kept_count: currentRules.length,
      weights_sample: ranked.slice(0, Math.min(5, overflow)).map(r => ({ w: r.w })),
    });
  }
  ...
  log("consolidation_complete", {
    ...
    threshold,
    // Section 5 observability — detect implicit-spam or weight drift.
    weight_sum_by_promotable: promotable.map(c => Number(clusterWeight(c).toFixed(2))),
    explicit_count: corrections.filter(c => !c.is_implicit).length,
    implicit_count: corrections.filter(c => c.is_implicit).length,
  });

  // Auto-backtest: dual-threshold — composite score + collapse index
  // Revert if score drops significantly OR if collapse index drops while score is mediocre
  let backtestResult = "skipped";
  if (rulesPromoted > 0) {
    try {
      const scoreAfter = await calculateFidelityScore(personaId, { client });
      if (scoreAfter && scoreBefore) {
        const scoreDelta = scoreAfter.score_global - scoreBefore.score_global;
        const collapseBefore = scoreBefore.collapse_index ?? null;
        const collapseAfter = scoreAfter.collapse_index ?? null;
        const collapseDelta = (collapseBefore !== null && collapseAfter !== null)
          ? collapseAfter - collapseBefore : null;

        log("consolidation_backtest", {
          persona: personaId,
          score_before: scoreBefore.score_global,
          score_after: scoreAfter.score_global,
          score_delta: scoreDelta,
          collapse_before: collapseBefore,
          collapse_after: collapseAfter,
          collapse_delta: collapseDelta,
        });

        // Dual-threshold revert conditions:
        // 1. Score dropped significantly
        // 2. Collapse index dropped significantly AND score is below excellence threshold
        const scoreRevert = scoreDelta < BACKTEST_REVERT_SCORE_DELTA;
        const collapseRevert = collapseDelta !== null
          && collapseDelta < BACKTEST_REVERT_COLLAPSE_DELTA
          && scoreAfter.score_global < BACKTEST_COLLAPSE_FLOOR;

        if (scoreRevert || collapseRevert) {
          const reason = scoreRevert ? "score_drop" : "collapse_variance";
          for (const rule of promotedRules) {
            await revertConsolidation(personaId, rule);
          }
          clearIntelligenceCache(intellId);
          backtestResult = `auto-reverted:${reason}`;
          log("consolidation_auto_reverted", {
            persona: personaId,
            reason,
            score_delta: scoreDelta,
            collapse_delta: collapseDelta,
          ...
          logLearningEvent(personaId, "consolidation_reverted", {
            reason,
            delta_score: scoreDelta,
            delta_collapse: collapseDelta,
            reverted_rules: promotedRules,
          }, {
            fidelity_before: scoreBefore.score_global,
            fidelity_after: scoreAfter.score_global,
            collapse_before: collapseBefore,
            collapse_after: collapseAfter,
          });
        ...
        logLearningEvent(personaId, "consolidation_run", {
          promoted: rulesPromoted,
          rules: promotedRules,
          cluster_sizes: promotable.map(c => c.members.length),
          threshold,
          revert: false,
        }, {
          fidelity_before: scoreBefore.score_global,
      ...
      log("consolidation_backtest_error", { error: err.message });
      backtestResult = "error";
    }
  }

  // Fallback: emit a consolidation_run event when backtest didn't already emit one
  // (scoreBefore null, scoreAfter null, or backtest error).
  if (rulesPromoted > 0 && backtestResult !== "passed") {
    logLearningEvent(personaId, "consolidation_run", {
      promoted: rulesPromoted,
      rules: promotedRules,
      cluster_sizes: promotable.map(c => c.members.length),
