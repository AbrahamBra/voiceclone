 ...
 *
 * Rule shape: { id, category, locked, weight, detect(text, ctx), reason, shadow? }
 * - detect returns true if the rule is VIOLATED.
 * - shadow:true → violation is logged in shadowViolations but does NOT contribute
 *   to violationScore / maxScore. Used for new rules under empirical observation.
 */
...
export const SETTER_RULES = [
  ...
  {
    // B5 — Vouvoiement par défaut, miroir si lead initie tutoiement.
    // Override : persona.voice.tutoiement_default=true (préférence documentée à
    // l'onboarding du clone). Sans override → vouvoiement par défaut.
    // Shadow : on observe le taux de fire en prod avant de bloquer.
    id: "B5", category: "ton", locked: false, weight: 0.5, shadow: true,
    detect: (t, ctx) => {
      // shadow phase: regex restreint à pronoms sujets non-homonymes pour éviter "le ton", "ta tata"
      const TU_CLONE = /\b(tu|toi|t')\b/i;
      const TU_LEAD = /\b(tu|te|toi|t'|ton|ta|tes)\b/i;
      const VOUS = /\b(vous|votre|vos)\b/i;
      const cloneTu = TU_CLONE.test(t);
      if (!cloneTu) return false;
      const lead = ctx?.priorLeadMessage || "";
      if (TU_LEAD.test(lead)) return false;     // miroir : lead tutoie → OK
      if (VOUS.test(lead)) return true;         // miroir miss : lead vouvoie
      return !ctx?.persona?.tutoiement_default; // lead muet → applique défaut
    },
    reason: "tutoiement sans signal du lead (vouvoiement par défaut)",
  },
 ...
 *                         priorLeadMessage?: string, persona?: { tutoiement_default?: boolean } }
 * @returns {{ violations: Array, violationScore: number, maxScore: number,
 *             shadowViolations: Array }}
 */
export function evaluateSetterBaseline(text, ctx = {}) {
  ...
  for (const rule of SETTER_RULES) {
    ...
    if (!active) continue;
    const isShadow = !!rule.shadow;
    if (!isShadow) maxScore += rule.weight;
    try {
      if (rule.detect(text, ctx)) {
        const v = { id: rule.id, category: rule.category, reason: rule.reason, weight: rule.weight, locked: rule.locked };
        if (isShadow) {
          shadowViolations.push(v);
        } else {
          violations.push(v);
  ...
  return { violations, violationScore, maxScore, shadowViolations };
}
