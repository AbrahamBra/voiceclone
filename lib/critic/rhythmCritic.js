...
/**
 * Persiste un résultat déjà calculé dans rhythm_shadow.
 * Returns the inserted row id so the caller can later link it to a message_id
 * once the assistant message has been persisted (chat.js does this).
 */
export async function persistShadow({ personaId, conversationId = null, messageId = null, draft, result }) {
  if (!personaId || !draft || !supabase || !result) return null;
  const { data, error } = await supabase.from("rhythm_shadow").insert({
