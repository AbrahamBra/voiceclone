...
export async function extractGraphKnowledge(personaId, correctionText, botMsg, userMsg, client) {
  ...
  try {
    ...
    try {
      ...
      log("graph_extraction_json_error", { persona: personaId, error: parseErr.message });
      return EMPTY;
    }
    ...
    if (graphData.new_entities?.length > 0) {
      ...
      if (inserted?.length > 0) {
        try {
          ...
          log("entity_embed_inline_error", { error: embErr.message });
        }
      }
      ...
      if (inserted?.length > 0 && graphData.new_relations?.length > 0) {
        ...
        if (relationRows.length > 0) {
          ...
          if (relErr) log("relation_insert_error", { persona: personaId, error: relErr.message });
        }
      }
      ...
      if (contradictions.length > 0) {
        ...
        for (const c of contradictions) {
          ...
          if (fromEnt && toEnt && fromEnt.id !== toEnt.id) {
            ...
            if (contrErr) log("contradiction_insert_error", { persona: personaId, error: contrErr.message });
          }
        }
      }
    }
    ...
    log("graph_extraction_error", { persona: personaId, error: e.message });
  ...
  try {
    ...
    if (inserted?.length > 0) {
      try {
        ...
        log("entity_embed_error", { persona: personaId, error: embErr.message });
      }
    }

    log("content_entities_extracted", {
      persona: personaId,
      source: sourcePath,
