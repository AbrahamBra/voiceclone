...
import { logRhythmShadow } from "./critic/rhythmCritic.js";
...
async function awaitFinalMessageOrTimeout(stream, getCurrentText, timeoutMs, label) {
  ...
  try {
    ...
    if (result && result.__timeout) {
      console.log(JSON.stringify({ event: `${label}_timeout`, ts: new Date().toISOString(), ms: timeoutMs, streamed_chars: getCurrentText().length }));
      return {
    ...
    console.log(JSON.stringify({ event: `${label}_error`, ts: new Date().toISOString(), error: err?.message || String(err), streamed_chars: getCurrentText().length }));
    return {
      stop_reason: "error",
      content: [{ type: "text", text: getCurrentText() }],
      usage: { input_tokens: 0, output_tokens: 0 },
...
export async function runPipeline({ systemPrompt, messages, sse, res, voiceRules, corrections, apiKey, model, personaId, conversationId, rhythmCtx }) {
  ...
  // PASS 2c: RhythmCritic shadow log (non-blocking, fire-and-forget).
  // DOES NOT gate anything — pure observation until precision/recall validated.
