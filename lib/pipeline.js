...
export async function runPipeline({ systemPrompt, messages, sse, res, voiceRules, corrections, apiKey, model, personaId, conversationId, rhythmCtx, sources }) {
  ...
  if (personaId && currentText.length > 10) {
    ...
    if (criticResult) {
      // Awaited (not fire-and-forget) so chat.js can link the assistant message_id
      // to this shadow row after persisting the message. Pre-fix, persistShadow
      // ran detached → message_id was always null → critic-prod-coverage = 14%.
      shadowRowIdPromise = persistShadow({
        personaId,
