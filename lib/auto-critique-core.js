// Core logic for N3 auto-critique. Used by both the user endpoint
// (api/auto-critique.js) and the scheduled cron (api/cron-auto-critique.js).

import Anthropic from "@anthropic-ai/sdk";
import { supabase } from "./supabase.js";
import { getCorrectionsFromDb } from "./knowledge-db.js";
import { getActiveHardRules } from "./protocol-db.js";
import { logLearningEvent } from "./learning-events.js";

const anthropic = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });

export async function critiquePersona(personaId) {
  ...
  if (hardRules.length > 0) {
    rulesLines.push(
      hardRules
        .map((r, i) => `${i + 1}. ${r.rule || r.content || JSON.stringify(r)}`)
        .join("\n"),
    );
  }

  if (rulesLines.length === 0) {
    return { critiques: [], emitted: 0, skipped: true, reason: "no_active_rules" };
  }
  ...
  if (botTurns.length === 0) {
    return { critiques: [], emitted: 0, skipped: true, reason: "no_recent_output" };
  }

  const messagesText = botTurns
...
Réponds uniquement avec le JSON, sans texte autour.`;
  ...
  return { critiques, emitted, ...(parseError ? { parse_error: true } : {}) };
}
