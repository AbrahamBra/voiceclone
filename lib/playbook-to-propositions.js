 ...
 *                                   Reasons: 'toggle_too_short' (length filter),
 *                                   'extractor_returned_empty' (LLM returned legitimate []).
 * @param {Function} [opts.onError] - callback({toggle_idx, toggle_title, sub_chunk_idx?, message, retryable, chunk_len})
 *                                    fired when the Sonnet call FAILS (timeout, 429, 5xx, etc.).
 *                                    Distinct from onSkip (legitimate empty). PR #232 — visibility fix.
  ...
  for (const toggle of parsed.toggles) {
    ...
    if (proseLen > MAX_TOGGLE_LEN) {
      ...
      for (const sub of subChunks) {
        ...
        if (!errored && (!cands || cands.length === 0)) {
          onSkip({
            reason: "extractor_returned_empty",
            toggle_idx: toggle.idx,
            toggle_title: toggle.title,
            sub_chunk_idx: subIdx,
            prose_len: sub.length,
          });
        }
