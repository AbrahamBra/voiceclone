...
export async function aggregateMetrics(personaId, date = null) {
  ...
  if (error) {
    log("metrics_upsert_error", { persona: personaId, date: targetDate, error: error.message });
    return null;
  }

  log("metrics_aggregated", { persona: personaId, date: targetDate, messages: totalMessages });
  return metrics;
}
...
export async function aggregateAllMetrics(date = null) {
  ...
  for (const p of personas) {
    try {
      ...
      log("metrics_persona_error", { persona: p.id, error: err.message });
    }
  }
