...
const BUSINESS_TERMS = /\b(client|équipe|equipe|biz|marché|marche|portfolio|boîte|boite|saas|startup|produit|chiffre|CA|revenu|mission|prestation|presta|secteur|vertical|cible|investissement|acquisition|immobilier|appartement|diversification|crypto|actions)\b/i;
...
function detectBusinessContext(msg, heatRow) {
  if (msg.role !== "user") return null;
  if (wordCount(msg.content) < 25) return null;
...
function detectGhost(messages, now) {
  // Scan for any bot message followed by a gap ≥ 48h (to next msg or to now).
  // Emit the MOST RECENT such gap (historical or current).
  let lastGhostBot = null;
  let lastGhostGapMs = 0;
  for (let i = 0; i < messages.length; i++) {
    const m = messages[i];
    if (m.role !== "bot") continue;
    const next = messages[i + 1];
    const start = new Date(m.created_at).getTime();
    const end = next ? new Date(next.created_at).getTime() : now.getTime();
    const gap = end - start;
    if (gap >= GHOST_MS) {
      lastGhostBot = m;
      lastGhostGapMs = gap;
  ...
  if (!lastGhostBot) return null;
  const days = Math.floor(lastGhostGapMs / (24 * 3600 * 1000));
  return {
    kind: "ghost_2days",
    label: days >= 7 ? `${days} jours sans réponse` : "2+ jours de silence",
    quote: `${days} jour${days > 1 ? "s" : ""} entre ${new Date(lastGhostBot.created_at).toLocaleDateString("fr-FR")} et la réponse`,
    polarity: "neg",
    delta: 0.08,
    when: lastGhostBot.created_at,
    message_id: null,
  };
}

function detectRelanceUnanswered(messages) {
  // Find the longest streak of consecutive bot messages in the whole conversation.
  let longest = 0;
  ...
  const delta = Math.min(MAX_GHOST_DELTA, 0.05 * longest);
  return {
    kind: "relance_unanswered",
    label: `${longest} relances sans réponse`,
    quote: `${longest} messages envoyés, pas de retour`,
    polarity: "neg",
    delta,
    when: longestStart.created_at,
    message_id: null,
  };
}
...
export function extract({ messages = [], heatRows = [], now = new Date(), limit = 8 } = {}) {
  ...
  // Sort: newest first by timestamp; for same-timestamp signals, tiebreak by
  // kind priority (spec §Conflict resolution — lower index = higher priority).
...
// Conflict-resolution priority for signals sharing a timestamp.
// See spec 2026-04-18-thermometre-rail-design.md §Conflict resolution.
const KIND_PRIORITY = [
  "books_slot",
  "accept_call",
  "propose_call",
...
const KIND_RANK = Object.fromEntries(KIND_PRIORITY.map((k, i) => [k, i]));
