...
/**
 * Returns active artifacts for the persona's active doc (Chantier 2bis).
 ...
 * @param {object} [opts]
 * @param {number} [opts.limit=30] max artifacts to return
 * @returns {Promise<Array<{id:string, source_section_id:string, kind:string, content:object, severity:string}>>}
 */
export async function getActiveArtifactsForPersona(sb, personaId, { limit = 30 } = {}) {
  const doc = await getActiveDocument(sb, personaId);
  if (!doc) return [];
