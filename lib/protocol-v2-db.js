...
import { partitionByDecay } from "./protocol-v2-decay.js";
 ...
 * log firings against them. Priority order: manual overrides first, then
 * hard > strong > light severity, then most-recent within the same tier.
 * This ensures foundational rules are never crowded out by newer light ones.
 *
...
export async function getActiveArtifactsForPersona(sb, personaId, { limit = 30, sourceCore = null } = {}) {
  ...
  // Fetch generously so JS priority-sort has the full candidate pool.
  // DB limit is a safety cap only (not a selection mechanism).
  const fetchCap = Math.max(limit * 6, 200);
  const { data: rawArtifacts } = await sb
    .from("protocol_artifact")
    .select("id, source_section_id, kind, content, severity, is_manual_override, stats, created_at")
    .in("source_section_id", sectionIds)
    .eq("is_active", true)
    .order("created_at", { ascending: false })
    .limit(fetchCap);

  // Priority sort: manual overrides first (never evicted), then hard > strong > light,
  // then most-recent within the same tier. Ensures foundational rules are not
  // crowded out by a burst of newer light patterns.
  const SEV_RANK = { hard: 3, strong: 2, light: 1 };
  const prioritized = (rawArtifacts || []).slice().sort((a, b) => {
    if (a.is_manual_override !== b.is_manual_override) return a.is_manual_override ? -1 : 1;
    const sw = (SEV_RANK[b.severity] || 0) - (SEV_RANK[a.severity] || 0);
    if (sw !== 0) return sw;
    return new Date(b.created_at) - new Date(a.created_at);
  });

  // Apply decay : strip stale non-manual non-hard artifacts that have not
  // fired in STALE_AFTER_DAYS. Hard rules + manual overrides are kept.
  // Cf lib/protocol-v2-decay.js for the rule.
  const { active, decayed } = partitionByDecay(prioritized);
  if (decayed.length > 0) {
    console.log(JSON.stringify({
      event: "protocol_v2_decay_applied",
      ts: new Date().toISOString(),
      persona: personaId,
      kept: active.length,
      stale: decayed.length,
    }));
  }
