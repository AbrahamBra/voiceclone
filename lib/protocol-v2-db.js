...
export async function getActiveArtifactsForPersona(sb, personaId, { limit = 30, sourceCore = null } = {}) {
  ...
  // Pull the fields needed both for the prompt and for decay filtering.
  // We over-fetch slightly (limit * 2) so that decay can prune without
  // starving the prompt: if many artifacts are stale, we still want `limit`
  // active ones returned.
  ...
  const { active, decayed } = partitionByDecay(rawArtifacts || []);
  if (decayed.length > 0) {
