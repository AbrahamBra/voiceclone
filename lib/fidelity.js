 ...
 * Used as fallback when Haiku labelling fails — takes the 2 most frequent
 * non-trivial words, capitalizes the first, returns "Mot1 mot2".
 * Returns null when the samples yield no usable tokens.
...
export function kmeansSelectRepresentatives(embeddings, posts, { k = 3, repsPerCluster = 3, maxIter = 20 } = {}) {
  ...
  for (let iter = 0; iter < maxIter; iter++) {
    ...
    // Recompute centroids (arithmetic mean in embedding space)
  ...
  // Pick repsPerCluster posts closest to each centroid
