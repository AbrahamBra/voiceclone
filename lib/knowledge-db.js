...
import { retrieveChunks, buildRagContext } from "./rag.js";
...
/**
 * Find relevant knowledge files via keyword matching + embedding fallback.
 * Hybrid approach: keywords first (instant), embeddings if no match.
 */
export async function findRelevantKnowledgeFromDb(personaId, messages) {
  ...
  // Phase 2: If no keyword match, fallback to semantic search
  if (matched.length === 0 && isEmbeddingAvailable()) {
    try {
      const query = messages.slice(-3).map((m) => m.content).join(" ");
      const chunks = await retrieveChunks(supabase, personaId, query);
      const context = buildRagContext(chunks);
      if (context) {
        matched.push({ path: "rag-retrieval", content: context });
      }
    } catch (err) {
      console.log(JSON.stringify({ event: "rag_fallback_error", error: err.message }));
    }
  }
