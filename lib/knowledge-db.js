...
async function loadPersonaData(personaId) {
  ...
  const { data: corrections } = await supabase
    .from("corrections").select("id, correction, user_message, bot_message, confidence, status, graduated_rule, created_at")
...
export async function findRelevantKnowledgeFromDb(personaId, messages, boostTerms = []) {
  ...
  if (isEmbeddingAvailable()) {
    try {
      ...
      log("rag_error", { error: err.message });
    }
  }
  ...
  // Dedup RAG chunks by source path: previously, when no keyword hit existed
  // for a given sourcePath, the key fell back to `rag:${item.path}` — but
  // path === sourcePath in that case, so chunks still collapsed. The real
  // bug was that only the FIRST chunk's content survived (`if !contentMap[key]`),
  // even though additional chunks bumped the RRF score. We now concat up to
  // RAG_CHUNKS_PER_SOURCE chunks per document so multi-chunk hits actually
  // contribute their content, not just their score.
  const RAG_CHUNKS_PER_SOURCE = 3;
  ragResults.forEach((item, rank) => {
    const key = item.sourcePath || `rag:${item.path}`;
    rrfScores[key] = (rrfScores[key] || 0) + 1 / (K + rank + 1);
    if (!contentMap[key]) {
      contentMap[key] = { path: item.path, content: item.content, sources: new Set(), ragChunks: 1 };
    } else if (!contentMap[key].sources.has("keyword") && (contentMap[key].ragChunks || 0) < RAG_CHUNKS_PER_SOURCE) {
      // Same source, additional RAG chunk → concat. Skip when the keyword
      // branch already populated the full document content for this key.
      contentMap[key].content += "\n\n---\n\n" + item.content;
      contentMap[key].ragChunks = (contentMap[key].ragChunks || 0) + 1;
    }
...
export async function findRelevantEntities(personaId, messages) {
  ...
  if (queryEmbedding) {
    ...
    try {
      ...
      log("entity_semantic_match_error", { error: err.message });
    }
  }
  ...
  // 5. Graph walk: 2-hop with relation-type weighting + hub penalty
  ...
  const MAX_HOP1_PER_SOURCE = 3; // hub penalty: cap neighbors per direct entity

  const hop1Map = new Map(); // entityId → { id, maxWeight }
  const hop1SourceCount = new Map(); // directEntityId → count of hop1 neighbors added
  const visited = new Set(directIds); // cycle awareness

  for (const rel of data.relations) {
    const weight = RELATION_WEIGHTS[rel.relation_type] ?? 0.5;
    if (weight === 0) continue; // skip contradicts

    let sourceId = null, targetId = null;
    if (directIds.has(rel.from_entity_id) && !visited.has(rel.to_entity_id)) {
      sourceId = rel.from_entity_id; targetId = rel.to_entity_id;
    } else if (directIds.has(rel.to_entity_id) && !visited.has(rel.from_entity_id)) {
      sourceId = rel.to_entity_id; targetId = rel.from_entity_id;
    }
    if (!sourceId) continue;

    // Hub penalty: max N hop-1 neighbors per source entity
    const srcCount = hop1SourceCount.get(sourceId) || 0;
    if (srcCount >= MAX_HOP1_PER_SOURCE) continue;

    const existing = hop1Map.get(targetId);
    if (!existing || weight > existing.maxWeight) {
      hop1Map.set(targetId, { id: targetId, maxWeight: weight });
    }
    hop1SourceCount.set(sourceId, srcCount + 1);
  }

  const hop1Ids = new Set(hop1Map.keys());
  for (const id of hop1Ids) visited.add(id);

  // 2nd hop: entities connected to 1-hop (lower priority, cycle-aware)
  ...
  // Combine: direct + weighted hop-1 + weighted hop-2, cap at 25
  const hop1Entities = eligible
    .filter(e => hop1Ids.has(e.id))
    .map(e => {
      const relWeight = hop1Map.get(e.id)?.maxWeight || 0.6;
      return { ...e, score: (e.score || e.confidence || 0.8) * 0.6 * relWeight };
    });
  ...
  // 6. Collect relations between matched entities (exclude contradicts from injection)
  ...
  if (directIds.size > 0) {
    ...
    supabase.from("knowledge_entities")
      ...
      .in("id", ids)
      .then(({ error }) => {
        if (error) log("entity_match_update_error", { error: error.message });
        // Update cache in-memory
        for (const e of data.entities) {
          if (directIds.has(e.id)) e.last_matched_at = now;
        }
      })
      .catch((err) => log("entity_match_update_error", { error: err.message }));
  }
