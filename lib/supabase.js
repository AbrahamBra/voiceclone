...
export async function authenticateRequest(req) {
  ...
  // Joined query: sessions + clients in one round-trip via PostgREST embed.
  // Previous implementation did two sequential fetches (session → client),
  // adding ~1 DB RTT to every authenticated API call.
  const sessionToken = req.headers["x-session-token"];
  if (sessionToken && supabase) {
    const { data: row } = await supabase
      .from("sessions")
      .select("expires_at, client:clients(*)")
      .eq("token", sessionToken)
      .single();

    if (row && new Date(row.expires_at) > new Date() && row.client) {
      const client = row.client;
      return { client, isAdmin: client.access_code === "__admin__" };
    }
...
// Anthropic SDK reports input_tokens (uncached) and cache_read_input_tokens
// as DISJOINT counts — pipeline.js aggregates them separately. Earlier code
// computed `inputTokens - cacheRead` as if they overlapped, producing
// negative cost_cents whenever cache hits exceeded uncached input tokens
// (very common with long cached system prompts and short user turns).
export function computeCostCents(inputTokens, outputTokens, cacheRead, model) {
  const pricing = MODEL_PRICING[model] || DEFAULT_PRICING;
  return ((inputTokens * pricing.input + cacheRead * pricing.cache_read + outputTokens * pricing.output) / 1_000_000) * 100;
}

export async function logUsage(clientId, personaId, inputTokens, outputTokens, { model, cacheRead = 0 } = {}) {
  if (!supabase || !clientId) return;

  const costCents = computeCostCents(inputTokens, outputTokens, cacheRead, model);
...
export async function hasPersonaAccess(clientId, personaId) {
  ...
  // Admin bypass — admin client is identified by access_code "__admin__"
  const { data: clientRow } = await supabase
    .from("clients").select("access_code").eq("id", clientId).single();
  if (clientRow?.access_code === "__admin__") return true;
...
// Vercel preview deployments for THIS project only. Anyone can deploy to
// *.vercel.app, so an open `.endsWith(".vercel.app")` lets evil-clone.vercel.app
// hit our API with the user's credentials. Restrict to previews of voiceclone-lake.
const VERCEL_PREVIEW_RE = /^https:\/\/voiceclone-lake(-[a-z0-9-]+)*\.vercel\.app$/;
