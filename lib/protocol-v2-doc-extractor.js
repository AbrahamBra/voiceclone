 ...
 * @param {Function} [opts.onError] — called with ({message, retryable, chunk_len}) when the
 *                                    Sonnet call fails (timeout, 429, 5xx, malformed JSON).
 *                                    Lets callers distinguish "LLM returned []" (legitimate empty)
 *                                    from "call failed" (recoverable). Return shape unchanged for
 *                                    back-compat — errors still resolve to []. PR #232.
...
export async function extractFromChunk(chunk, ctx = {}, opts = {}) {
  ...
  try {
    ...
    log("protocol_v2_doc_extractor_error", { message });
    if (typeof opts.onError === "function") {
      // Heuristic : timeout / 429 / 5xx are network-recoverable. Schema/validation errors are not.
      const retryable = /timeout|429|rate[_ ]limit|5\d\d|ECONNRESET|ETIMEDOUT|fetch failed/i.test(message);
      try {
        opts.onError({ message, retryable, chunk_len: trimmed.length });
      } catch {
        // never let an onError handler break the extractor
      }
    }
    return [];
  }
