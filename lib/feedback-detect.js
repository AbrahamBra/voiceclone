...
export async function classifyMessage(msg, lastBotMsg, client) {
  if (!msg || msg.length > 500) return "CHAT";

  // Fast-path: very short + regex match → trust the regex
  if (msg.length < 40) {
    if (NEGATIVE_PATTERN.test(msg)) return "NEGATIVE";
    if (CORRECTION_PATTERN.test(msg)) return "CORRECTION";
    if (VALIDATION_PATTERN.test(msg) && msg.length <= MAX_VALIDATION_LENGTH) return "VALIDATION";
...
Si aucune ne correspond, reponds {"indices": [], "reason": "aucune correspondance"}`,
    ...
    for (const idx of data.indices) {
      ...
      const { error: corrErr } = await supabase.from("corrections")
        ...
        .eq("id", c.id);
      if (corrErr) console.log(JSON.stringify({ event: "correction_demote_error", id: c.id, error: corrErr.message }));
      ...
      if (entities?.length) {
        ...
        for (const e of matched) {
          ...
          const { error: entErr } = await supabase.from("knowledge_entities")
            ...
            .eq("id", e.id);
          if (entErr) console.log(JSON.stringify({ event: "entity_demote_error", id: e.id, error: entErr.message }));
        }
      }
 ...
 */
export async function detectMetacognitiveInsights(intellId, userMsg, conversationMessages, botResponse, client) {
  if (!userMsg || userMsg.length <= MIN_METACOGNITIVE_LENGTH) return 0;

  // Exclude messages already handled by other detectors
  if (INSTRUCTION_PATTERN.test(userMsg)) return 0;
  if (CORRECTION_PATTERN.test(userMsg)) return 0;
  if (VALIDATION_PATTERN.test(userMsg)) return 0;
  if (NEGATIVE_PATTERN.test(userMsg)) return 0;
  ...
  try {
    ...
    console.log(JSON.stringify({
      event: "metacognitive_insights_detected",
      ts: new Date().toISOString(),
      persona: intellId,
    ...
    console.log(JSON.stringify({
      event: "metacognitive_insights_error",
      ts: new Date().toISOString(),
      persona: intellId,
      error: e.message,
    }));
