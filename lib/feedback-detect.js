...
import { resolveFirings } from "./protocol-v2-rule-counters.js";
import { log } from "./log.js";

/**
 * Chantier 3 leak fix : when the user emits an implicit coaching/negative
 * signal in chat-libre (CORRECTION pattern matched, NEGATIVE pattern matched),
 * resolve the protocol_rule_firing rows that were inserted pending when the
 * coached bot message was generated, marking them as `harmful`. Without this,
 * harmful_count was blind to chat-coaching ("trop long", "oublie cette règle")
 * and only reflected explicit "Corrected" button clicks. proposeRetirement
 * was therefore aveugle to the cases where rules consistently produced
 * outputs the user rejected via natural language.
 *
 * Best-effort : never throws to caller — the corrections write is the
 * primary contract of these detectors.
 */
async function resolveFiringsBestEffort({ messageId, outcome, source }) {
  if (!messageId || !outcome) return;
  try {
    await resolveFirings({ supabase, messageId, outcome });
  } catch (err) {
    log("feedback_detect_resolve_firings_error", { source, message: err?.message || String(err) });
  }
}
 ...
 */
const CORRECTION_PATTERN = /\b(trop\s+(long|court|formel|direct|agressif|commercial|verbeux|robotique|vague|générique|froid|chaud|familier|ia|construit|propre|lisse)|pas\s+assez\s+(direct|naturel|concis|percutant|punchy|personnel|chaleureux|formel)|plus\s+(court|long|direct|simple|naturel|concis|percutant|punchy)|moins\s+(formel|long|verbeux|commercial|agressif|robotique)|(?:c'?est|ca\s+fait|ça\s+fait)\s+(trop|pas|un\s+peu)\s+(formel|long|court|commercial|agressif|robotique|vague|froid|ia|nul)|c'?est\s+nul|nul\s+[àa]\s+chier|creux|refais|refais[\s-]le|ça\s+sonne\s+(ia|robot|faux|froid)|sonne\s+(ia|robot|artificiel)|enlève|enl[eè]ve|supprime|retire|évite|arr[eê]te\s+(les?|cette|ce|d'utiliser)|on\s+dit\s+pas|dis\s+pas|fais\s+pas|mets?\s+pas)\b|\bnul\b/i;

const MAX_CORRECTION_LENGTH = 250;
...
export function looksLikeCorrection(msg) {
  if (!msg || msg.length > MAX_CORRECTION_LENGTH) return false;
  return CORRECTION_PATTERN.test(msg);
}

/**
 * Out-of-target signals — user signals the prospect is not ICP.
 * The bot must STOP proposing messages and acknowledge.
 */
const HORS_CIBLE_PATTERN = /\b(hors[- ]cible|pas\s+(la\s+)?cible|mauvais\s+profil|pas\s+(le\s+)?bon\s+profil|pas\s+icp|ne\s+(correspond|match)\s+pas|stop\s+(les?\s+)?messages?|arr[eê]te\s+(de\s+)?(r[eé]diger|[eé]crire|envoyer)|plus\s+de\s+messages?|laisse\s+(tomber|[çc]a)|abandon(ne)?|pas\s+de\s+suite)\b/i;

export function looksLikeHorsCible(msg) {
  if (!msg || msg.length > 300) return false;
  return HORS_CIBLE_PATTERN.test(msg);
}
...
Reponds par UN seul mot parmi : INSTRUCTION CORRECTION VALIDATION NEGATIVE STOP CHAT
...
STOP = le prospect est hors-cible, l'utilisateur veut qu'on arrete de rediger des messages ("hors cible", "pas icp", "stop les messages", "laisse tomber", "mauvais profil")
...
export async function classifyMessage(msg, lastBotMsg, client) {
  ...
  if (msg.length < 40) {
    if (HORS_CIBLE_PATTERN.test(msg)) return "STOP";
  ...
  const anyRegex = INSTRUCTION_PATTERN.test(msg) || CORRECTION_PATTERN.test(msg) ||
    VALIDATION_PATTERN.test(msg) || NEGATIVE_PATTERN.test(msg) || HORS_CIBLE_PATTERN.test(msg);
  ...
  try {
    ...
    const valid = new Set(["INSTRUCTION", "CORRECTION", "VALIDATION", "NEGATIVE", "STOP", "CHAT"]);
...
Si aucune ne correspond, reponds {"indices": [], "reason": "aucune correspondance"}`,
    ...
    // Chantier 3 leak fix : the user wants to undo a rule that produced an
    // output they reject. Resolve pending firings on the last bot message
    // as harmful. Same rationale as detectCoachingCorrection. Non-blocking.
    if (demoted.length > 0) {
      const lastBotMsg = [...conversationMessages].reverse().find(m => m.role === "assistant" || m.role === "bot");
      if (lastBotMsg?.id) {
        await resolveFiringsBestEffort({
  ...
  try {
    ...
    for (const c of data.corrections) {
      ...
      await supabase.from("corrections").insert({
        ...
        source_channel: "direct_instruction",
      });
...
- "enleve les emojis" → {"rule": "Ne pas utiliser d'emojis dans les messages"}`,
    ...
    await supabase.from("corrections").insert({
      ...
      source_channel: "coaching_correction",
    });

    await extractGraphKnowledge(intellId, data.rule, lastBotMsg.content, userMsg, client);
    clearIntelligenceCache(intellId);

    // Chantier 3 leak fix : resolve pending firings on the coached bot
    // message as harmful (they likely produced the output the user is
    // correcting). Non-blocking.
    await resolveFiringsBestEffort({
      messageId: lastBotMsg.id,
      outcome: "harmful",
      source: "coaching_correction",
    });
  ...
  try {
    ...
    for (const c of data.corrections) {
      ...
      await supabase.from("corrections").insert({
        ...
        source_channel: "chat_correction",
      });
