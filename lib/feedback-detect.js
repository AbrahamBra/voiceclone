 ...
 */
const CORRECTION_PATTERN = /\b(trop\s+(long|court|formel|direct|agressif|commercial|verbeux|robotique|vague|générique|froid|chaud|familier)|pas\s+assez\s+(direct|naturel|concis|percutant|punchy|personnel|chaleureux|formel)|plus\s+(court|long|direct|simple|naturel|concis|percutant|punchy)|moins\s+(formel|long|verbeux|commercial|agressif|robotique)|(?:c'?est|ca\s+fait|ça\s+fait)\s+(trop|pas|un\s+peu)\s+(formel|long|court|commercial|agressif|robotique|vague|froid)|enlève|enl[eè]ve|supprime|retire|évite|arr[eê]te\s+(les?|cette|ce|d'utiliser)|on\s+dit\s+pas|dis\s+pas|fais\s+pas|mets?\s+pas)\b/i;
