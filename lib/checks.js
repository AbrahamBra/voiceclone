...
const AI_PATTERNS_FR = /tu as mis le doigt sur|r[ée]sultat\s*:|concr[eè]tement\s*:|c'est l[aà] que|et c'est exactement|la vraie question c'est/i;
...
export function checkResponse(text, voiceRules) {
  ...
  // AI French patterns (hard — trigger rewrite)
  if (AI_PATTERNS_FR.test(text)) {
    violations.push({ type: "ai_pattern_fr", severity: "hard", detail: text.match(AI_PATTERNS_FR)?.[0] });
  }
