 ...
 *
 * `ttr` is intentionally excluded — it already contributes to `styleScore`
 * (ttrSim, 18% weight) and to `computeCollapseIndex` directly (ttrRatio, 25%
 * weight). Counting it a third time pushed the effective TTR weight in the
 * collapse index to ~39%, swamping the other signals on short DM drafts.
 */
export function normalizedVarianceLoss(sourceMetrics, draftMetrics) {
  const relativeDims = ["sentenceLenStd", "kurtosis", "questionPlacementVar"];
  let totalLoss = 0;
  let count = 0;
